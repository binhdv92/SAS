%MACRO CHTSingleThinTest(Lineage);
OPTIONS MAUTOSOURCE MINOPERATOR NOMPRINT validvarname=any NOFULLSTIMER;
LIBNAME single '/sasdata/StudioTopLevel/Projects/S6_CHT_SingleTest';
LIBNAME singleth '/sasdata/StudioTopLevel/Projects/S6_CHT_SingleThinTest';
...
%MACRO Profile;
%IF %SUBSTR(&Lineage., 1, 4)=PGT1 %THEN %DO;
PROC SQL;
CREATE TABLE WORK.&lineage._ProfileData AS
SELECT * FROM SINGLE.ME_DEP_CHT_M8CIMData
WHERE Lineage="&Lineage." AND CIM='Y';
QUIT;
%END;

%IF &Lineage=PGT21B OR &Lineage=PGT21C OR &Lineage=PGT21D %THEN %DO;
PROC SQL;
CREATE TABLE WORK.&lineage._ProfileData AS 
SELECT * FROM SINGLE.ME_DEP_CHT_M8CIMData
...
%IF &Lineage=PGT21A OR %SUBSTR(&Lineage., 1, 3)=KMT %THEN %DO;
PROC SQL;
CREATE TABLE WORK.&lineage._ProfileData AS
SELECT * FROM SINGLE.ME_DEP_CHT_M8CIMData
...
filename pScore "/sasdata/StudioTopLevel/Projects/S6_CHT_SingleThinTest/&lineage._&&position&j.._zone&i._m&k..sas";
...
%PUT &Model1;
%CHECK(/sasdata/StudioTopLevel/Projects/S6_CHT_SingleThinTest/&Model1.);
...
%PUT &Model2;
%CHECK(/sasdata/StudioTopLevel/Projects/S6_CHT_SingleThinTest/&Model2.);
...
DATA singleth.&Lineage._SingleTh_FitStats;
