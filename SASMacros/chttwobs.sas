%MACRO chttwobs(Lineage);
OPTIONS MAUTOSOURCE MINOPERATOR NOMPRINT validvarname=any;
libname single '/sasdata/StudioTopLevel/Projects/S6_CHT_SingleTest';
libname cht2bs '/sasdata/StudioTopLevel/Projects/S6_CHT_TwoBS';
...
SELECT DISTINCT Lineage, StartTime, BeltSpeedSetpoint AS BeltSpeed, COUNT(BeltSpeedSetpoint) AS Number
...
SELECT DISTINCT Lineage, MAX(StartTime) AS EndTime, BeltSpeed, COUNT(BeltSpeed) AS Number
FROM WORK.OvenBeltSpeed
GROUP BY BeltSpeed
ORDER BY EndTime DESC;
QUIT;

DATA WORK.Count1;
SET WORK.Count(OBS=2);
cumulative+Number;
N=_N_;
RUN;

PROC SQL NOPRINT;
SELECT cumulative INTO: N FROM WORK.Count1 WHERE N=2; QUIT;
...
%IF &N.>=10 %THEN %DO;
DATA WORK.Speed;
SET WORK.Speed(OBS=2);
RUN;

PROC SQL;
CREATE TABLE WORK.&lineage._ProfileData AS
SELECT a.*
FROM SINGLE.ME_DEP_CHT_M8DataBad a
	INNER JOIN WORK.Speed b ON a.Lineage=b.Lineage AND ABS(a.BeltSpeedSetpoint-b.BeltSpeed) < 0.001
WHERE a.Lineage = "&Lineage.";
QUIT;
%END;
%ELSE %DO;
DATA WORK.Speed;
SET WORK.Speed(OBS=3);
...
select distinct a.*,b.&&position&j.._P_Mean, c.ProfileID, c.BeltSpeedSetpoint
...
filename pScore "/sasdata/StudioTopLevel/Projects/S6_CHT_TwoBS/&lineage._&&position&j.._zone&i._m&k..sas";

PROC REGSELECT DATA=PUBLIC.&&position&j.._Zone&i.;
MODEL &&position&j.._P_Mean = BeltSpeedSetpoint col1-col%SYSEVALF(12*&i-2) @2;
...
%PUT &Model1;
%CHECK(/sasdata/StudioTopLevel/Projects/S6_CHT_TwoBS/&Model1.);
...
%PUT &Model2;
%CHECK(/sasdata/StudioTopLevel/Projects/S6_CHT_TwoBS/&Model2.);
...
DATA cht2bs.&Lineage._Multi_FitStats;
SET WORK.FitStats_LL WORK.FitStats_L WORK.FitStats_C WORK.FitStats_R WORK.FitStats_RR;
BY Date Lineage Zone 'TC Location'n;
RUN;

DATA WORK.&Lineage._Multi_ParamEst;
SET WORK.ParamEst_LL WORK.ParamEst_L WORK.ParamEst_C WORK.ParamEst_R WORK.ParamEst_RR;
RUN;

PROC SQL;
CREATE TABLE cht2bs.&Lineage._Multi_ParamEst AS
SELECT a.Date, a.Lineage, a.Zone, a.'TC Location'n, a.Method, b.Parameter, b.Estimate, b.StdErr, b.tValue, b.Probt
FROM cht2bs.&Lineage._Multi_FitStats a
	LEFT JOIN WORK.&Lineage._Multi_ParamEst b ON a.Lineage = b.Lineage AND a.Zone = b.Zone AND a.'TC Location'n = b.'TC Location'n AND a.Method = b.Method
ORDER BY Date, Lineage, Zone, 'TC Location'n;
QUIT;
