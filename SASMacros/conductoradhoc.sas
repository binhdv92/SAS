...
%macro conductoradhoc(TableName, DaysAgo, Email, PGT1MES, PGT2MES, KMT1MES, KMT2MES, DMT1MES, DMT2MES, MFGODS, EDW, MAXPROD, CMS, PGTLAB, KMTLAB, DMTLAB, MAXIMO, GBLFED, DaysAgoDTS, DaysAgoPlant );
...
%if %sysfunc(exist(dppublic.&TableName.)) AND &DaysAgo > 0 %then %do;
	...
	%END;
%IF &PGT1MES = 1 AND &_Pgt1DsnActive = 1 %THEN %DO;	
	...
	%END;
	%END;
%IF &PGT2MES = 1 AND &_Pgt2DsnActive = 1 %THEN %DO;	
	...
	%END;
	%END;
%IF &KMT1MES = 1 AND &_Kmt1DsnActive = 1 %THEN %DO;
	...
	%END;
	%END;
%IF &KMT2MES = 1 AND &_Kmt2DsnActive = 1 %THEN %DO;
	...
	%END;
	%END;
%IF &DMT1MES = 1 AND &_Dmt1DsnActive = 1 %THEN %DO;	
	...
	%END;
	%END;
%IF &DMT2MES = 1 AND &_Dmt2DsnActive = 1 %THEN %DO;	
