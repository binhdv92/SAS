...
libname cht_thin '/sasdata/StudioTopLevel/Projects/S6_CHT_Single_ThinGls';
...
%MACRO Profile;
%IF &Lineage=PGT11B OR &Lineage=PGT12B %THEN %DO;
PROC SQL;
CREATE TABLE WORK.&lineage._ProfileData AS
SELECT * FROM SINGLE.ME_DEP_CHT_M8DataBad
WHERE Lineage="&Lineage." AND CIM='Y';
QUIT;
%END;

/*%IF &Lineage=PGT11A OR &Lineage=PGT12A %THEN %DO;
PROC SQL;
CREATE TABLE WORK.&lineage._ProfileData AS 
SELECT * FROM SINGLE.ME_DEP_CHT_M8DataBad
WHERE Lineage="&Lineage." AND ABS(BeltSpeedSetpoint-34.55)<0.001;
QUIT;
%END;*/

%IF %SUBSTR(&Lineage., 1, 4)=PGT2 OR &Lineage=PGT11A OR &Lineage=PGT12A %THEN %DO;
PROC SQL;
CREATE TABLE WORK.&lineage._ProfileData AS
SELECT * FROM SINGLE.ME_DEP_CHT_M8DataBad
WHERE Lineage="&Lineage." AND ABS(BeltSpeedSetpoint-35.53)<0.001;
QUIT;
%END;

%IF %SUBSTR(&Lineage., 1, 3)=DMT %THEN %DO;
PROC SQL;
CREATE TABLE WORK.&lineage._ProfileData AS
SELECT * FROM SINGLE.ME_DEP_CHT_M8DataBad
WHERE Lineage="&Lineage." AND ABS(BeltSpeedSetpoint-35.1)<0.001;
QUIT;
%END;

%IF %SUBSTR(&Lineage., 1, 3)=KMT %THEN %DO;
PROC SQL;
CREATE TABLE WORK.&lineage._ProfileData AS
SELECT * FROM SINGLE.ME_DEP_CHT_M8DataBad
WHERE Lineage="&Lineage." AND ABS(BeltSpeedSetpoint-32.54)<0.001;
QUIT;
%END;
%MEND;
...
SELECT *, 'Test' AS _ROLE_ LENGTH=8 FROM WORK.&&position&j..examplezone&i.&lineage
WHERE ProfileID = 1
ORDER BY ProfileID;
QUIT;
...
filename pScore "/sasdata/StudioTopLevel/Projects/S6_CHT_Single_ThinGls/&lineage._&&position&j.._zone&i._m&k..sas";
...
%PUT &Model1;
%CHECK(/sasdata/StudioTopLevel/Projects/S6_CHT_Single_ThinGls/&Model1.);
...
%PUT &Model2;
%CHECK(/sasdata/StudioTopLevel/Projects/S6_CHT_Single_ThinGls/&Model2.);
...
DATA cht_thin.&Lineage._SingleTh_FitStats;
