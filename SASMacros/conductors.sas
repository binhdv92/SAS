...
%mend;
%macro azureodsetl(TableName);
PROC SQL;
   CONNECT TO ODBC as con2
    (DATASRC="Azure_ODS" authdomain=EDWAuth);
...
%macro conductors(TableName, DaysAgo, Email, PGT1MES, PGT2MES, KMT1MES, KMT2MES, DMT1MES, DMT2MES, MFGODS, EDW, MAXPROD, CMS, PGTLAB, KMTLAB, DMTLAB, MAXIMO, GBLFED, AzureODS, DaysAgoDTS, DaysAgoPlant );
...
%IF &GBLFED = 1 %THEN %DO;	
	...
	%END;
	%END;
%IF &AzureODS = 1 %THEN %DO;	
	%LET PlantTableName = AODS_&TableName;
	%azureodsetl(&PlantTableName);
	%if %sysfunc(exist(work.&PlantTableName.)) %then %do;
		%LET ConCatString = &ConCatString work.&PlantTableName;
	%END;
	%ELSE %DO;
