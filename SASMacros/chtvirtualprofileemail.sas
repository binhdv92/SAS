...
%LET TimeHorizon = 24; /*This is how many hours from 'Now' we are allowed to find a 'validperiod' for the virtual CHT Profile*/
...
%IF %SUBSTR(&Lineage, 1, 3) = PGT %THEN %DO; %LET Now = &PGTTime; %END;
%IF %SUBSTR(&Lineage, 1, 3) = DMT %THEN %DO; %LET Now = &DMTTime; %END;
%IF %SUBSTR(&Lineage, 1, 3) = KMT %THEN %DO; %LET Now = &KMTTime; %END;
...
/*CHT Valid Period*/
%deletedsifexists(casuser,CHTValidPeriod); /*add this after you use the table to delete the table*/

PROC FEDSQL sessref=mysession;
CREATE TABLE casuser.CHTValidPeriod AS
SELECT a.*
FROM
                public.ME_DEP_CHTProdMix a
                INNER JOIN 
                (
                SELECT 
                                MAX(ChtBcrReadTime12) as MRRT
                                ,SourceLocation
                FROM public.ME_DEP_CHTProdMix        
                WHERE 
                                (
                                   (SourceLocation like 'PGT%' AND ChtBcrReadTime12 >= (&PGTTime-3600*24))
                                OR (SourceLocation like 'KMT%' AND ChtBcrReadTime12 >= (&KMTTime-3600*24))
                                OR (SourceLocation like 'DMT%' AND ChtBcrReadTime12 >= (&DMTTime-3600*24))
                                )
                                AND ValidPeriod='Yes'
								AND TotalGood+TotalWS > 37
                GROUP BY SourceLocation
                ) b on a.sourcelocation = b.sourcelocation and a.ChtBcrReadTime12 = b.MRRT;
QUIT;

DATA WORK.Valid;
SET casuser.CHTValidPeriod;
WHERE SUBSTR(SourceLocation, 1, 6) = "&Lineage.";
RUN;

Title HEIGHT=3 "CHT Product Status";
PROC REPORT DATA=WORK.Valid 
STYLE(REPORT) = {OUTPUTWIDTH=100%}
STYLE(HEADER) =	[JUST = CENTER FONT_FACE = calibri FONT_SIZE = 3 FOREGROUND = Black BACKGROUND = Orange PROTECTSPECIALCHARS=OFF];
COLUMN SourceLocation CHTBcrReadTime12 RampGoodModules RampWorkingScrap SoakGoodModules SoakWorkingScrap TotalPrevious2Hours;
DEFINE SourceLocation / "Line" DISPLAY ORDER=DATA STYLE(COLUMN)=[JUST=l FONT_FACE = calibri FONT_SIZE = 3];
DEFINE CHTBcrReadTime12 / "ReadTime12" DISPLAY STYLE(COLUMN)=[JUST=l FONT_FACE = calibri FONT_SIZE = 3];
DEFINE RampGoodModules / "Ramp Good Modules" DISPLAY STYLE(COLUMN)=[JUST=l FONT_FACE = calibri FONT_SIZE = 3];
DEFINE RampWorkingScrap / "Ramp Working Scrap" DISPLAY STYLE(COLUMN)=[JUST=l FONT_FACE = calibri FONT_SIZE = 3];
DEFINE SoakGoodModules / "Soak Good Modules" DISPLAY STYLE(COLUMN)=[JUST=l FONT_FACE = calibri FONT_SIZE = 3];
DEFINE SoakWorkingScrap / "Soak Working Scrap" DISPLAY STYLE(COLUMN)=[JUST=l FONT_FACE = calibri FONT_SIZE = 3];
DEFINE TotalPrevious2Hours / "Total Previous 2 Hours" DISPLAY STYLE(COLUMN)=[JUST=l FONT_FACE = calibri FONT_SIZE = 3];
QUIT;
TITLE;

DATA _NULL_;
SET WORK.VALID;
CALL SYMPUT('ValidTime', CHTBcrReadTime12);
RUN;

%PUT &ValidTime;

PROC SQL NOPRINT; SELECT COUNT(SourceLocation) INTO: Count FROM WORK.Valid; QUIT;
%PUT &Count;
...
SELECT
	...
	,VP_Pass
	,NextProfileSkipable
	,BeltSpeed FORMAT=BEST6.2
	,TargetSoak
	,Technology
	,TimeStamp
	,(CASE
		WHEN SUBSTR(EquipmentName, 1, 3) = 'PGT' THEN TimeStamp 
		WHEN SUBSTR(EquipmentName, 1, 3) = 'KMT' THEN (tzoneu2s(tzones2u(TimeStamp), 'Asia/Singapore'))
		WHEN SUBSTR(EquipmentName, 1, 3) = 'DMT' THEN (tzoneu2s(tzones2u(TimeStamp), 'Asia/Saigon'))
	END) AS LocalTime FORMAT=DATETIME.
	,(CASE WHEN SkipDecision = 'Yes' THEN 'SKIP PROFILE' ELSE 'RUN PROFILE' END) AS SDSubject
FROM CHTEMAIL.ME_DEP_CHT_SkipDecision
WHERE SUBSTR(EquipmentName, 1, 6) = "&Lineage." AND Oven_Instability = 0;
QUIT;

PROC SQL NOPRINT;
SELECT COUNT(EquipmentName) INTO: NL FROM WORK.Summary WHERE SUBSTR(EquipmentName, 1, 6)="&Lineage."; QUIT;

%IF &NL>0 %THEN %DO;
PROC SQL;
CREATE TABLE WORK.Summary1 AS
SELECT DISTINCT *, ABS(LocalTime - &ValidTime) AS TimeDiff
FROM WORK.Summary
GROUP BY EquipmentName
HAVING TimeDiff = MIN(TimeDiff);
QUIT;
%END;
...
SELECT
	...
	,(CASE
		...
		WHEN SUBSTR(EquipmentName,1,3) = 'DMT' THEN tzoneu2s(tzones2u(TimeStamp),'Asia/Saigon')
	END) AS LocalTime FORMAT=datetime.
	,(CASE WHEN SkipDecision = 'Yes' THEN 'SKIP PROFILE'
				ELSE 'RUN PROFILE'
			END) AS SDSubject
	,1 AS TimeDiff
...
MERGE CHTEMAIL.ME_DEP_CHT_M8BHistory WORK.Summary1(DROP=LocalTime TimeDiff);
BY EquipmentName TimeStamp;
RUN;
...
PROC REPORT DATA=WORK.Summary1
STYLE(REPORT) = {OUTPUTWIDTH=100%}
STYLE(HEADER) =	[JUST = CENTER FONT_FACE = calibri FONT_SIZE = 3 FOREGROUND = Black BACKGROUND = Orange PROTECTSPECIALCHARS=OFF];
COLUMN EquipmentName SkipDecision Model Model_Health Ramp Soak Oven_Instability VP_Fail VP_Pass NextProfileSkipable BeltSpeed LocalTime;
...
DEFINE LocalTime / DISPLAY STYLE(COLUMN)=[JUST=l FONT_FACE = calibri FONT_SIZE = 3];
...
ENDCOMP;
COMPUTE VP_Fail;
IF VP_Fail <= 2 THEN CALL DEFINE(_COL_, "STYLE", "STYLE={BACKGROUND=CX00FF00}");
IF 2 < VP_Fail <= 5 THEN CALL DEFINE(_COL_, "STYLE", "STYLE={BACKGROUND=CXFFFF00}");
...
ENDCOMP;
COMPUTE LocalTime;
IF INTNX('MINUTE', &Now., -30) <= LocalTime <= &Now THEN CALL DEFINE(_COL_, "STYLE", "STYLE={BACKGROUND=CX00FF00}");
IF LocalTime < INTNX('MINUTE', &Now., -30) THEN CALL DEFINE(_COL_, "STYLE", "STYLE={BACKGROUND=CXFFFF00}");
ENDCOMP;
QUIT;
TITLE;
...
PROC SQL;
CREATE TABLE WORK.PlotTime AS
SELECT DISTINCT a.Lineage, a.ReadTime15, ABS(a.ReadTime15 - b.LocalTime) AS TimeDiff
FROM PUBLIC.ME_DEP_CHT_CombineVP24 a
	LEFT JOIN WORK.Summary1 b ON a.Lineage = SUBSTR(b.EquipmentName, 1, 6)
GROUP BY a.Lineage
HAVING Lineage = "&Lineage." AND TimeDiff = MIN(TimeDiff);
QUIT;
...
SELECT 
	...
	,a.Zone
	,a.ReadTime15
    ,(a.UCL - a.Target) AS UB
    ,(a.LCL - a.Target) AS LB
    ,(CASE WHEN a.'TC Location'n = 'LL' THEN a.Delta ELSE . END) AS LL_Delta
    ,(CASE WHEN a.'TC Location'n = 'L' THEN a.Delta ELSE . END) AS L_Delta
    ,(CASE WHEN a.'TC Location'n = 'C' THEN a.Delta ELSE . END) AS C_Delta
    ,(CASE WHEN a.'TC Location'n = 'R' THEN a.Delta ELSE . END) AS R_Delta
    ,(CASE WHEN a.'TC Location'n = 'RR' THEN a.Delta ELSE . END) AS RR_Delta
FROM PUBLIC.ME_DEP_CHT_COMBINEVP24 a
	LEFT JOIN WORK.PlotTime b ON a.Lineage = b.Lineage AND a.ReadTime15 = b.ReadTime15
WHERE a.Lineage = "&Lineage." AND a.ReadTime15 = b.ReadTime15
ORDER BY a.Zone;
QUIT;
...
SELECT
	...
	,a.R
	,a.RR
FROM CHTEMAIL.ME_DEP_CHT_CombineVPMatrix a
	LEFT JOIN WORK.PlotTime b ON a.Lineage = b.Lineage AND a.ReadTime15 = b.ReadTime15
WHERE a.Lineage = "&Lineage." AND a.ReadTime15 = b.ReadTime15
ORDER BY a.Zone;
QUIT;
...
DATA CHTEMAIL.ME_DEP_CHT_M8BHistory;
MERGE CHTEMAIL.ME_DEP_CHT_M8BHistory WORK.Summary1(DROP = LocalTime);
BY EquipmentName TimeStamp;
RUN;
...
PROC SQL NOPRINT; SELECT COUNT(Lineage) INTO: N FROM WORK.Virtual15Min; QUIT;

%IF &N=0 %THEN %DO;
%LET TimeStamp=%SYSFUNC(DATETIME(), DATETIME23.);
%END;
...
%LET TimeStamp2 = %SYSFUNC(COMPRESS(&TimeStamp));
%PUT &TimeStamp2;
...
%IF %SYSFUNC(EXIST(WORK.Virtual15Table)) %THEN %DO;
PROC REPORT DATA=WORK.Virtual15Table
STYLE(REPORT) = {OUTPUTWIDTH=100%}
STYLE(HEADER) =	[JUST = CENTER FONT_FACE = calibri FONT_SIZE = 3 FOREGROUND = Black BACKGROUND = Orange PROTECTSPECIALCHARS=OFF];
COLUMN Lineage Zone LL L C R RR;
...
%ELSE %DO;
DATA WORK.Virtual15Table;
LENGTH Lineage $6. Zone 3. LL 8 L 8 C 8 R 8 RR 8;
RUN;
...
STYLE(HEADER) =	[JUST = CENTER FONT_FACE = calibri FONT_SIZE = 3 FOREGROUND = Black BACKGROUND = Orange PROTECTSPECIALCHARS=OFF];
COLUMN Lineage Zone LL L C R RR;
DEFINE Lineage / DISPLAY STYLE(COLUMN)=[JUST=l FONT_FACE = calibri FONT_SIZE = 3 CELLWIDTH=10%];
DEFINE Zone / DISPLAY STYLE(COLUMN)=[JUST=l FONT_FACE = calibri FONT_SIZE = 3 CELLWIDTH=5%];
DEFINE LL / DISPLAY STYLE(COLUMN)=[JUST=l FONT_FACE = calibri FONT_SIZE = 3 CELLWIDTH=17%];
DEFINE L / DISPLAY STYLE(COLUMN)=[JUST=l FONT_FACE = calibri FONT_SIZE = 3 CELLWIDTH=17%];
DEFINE C / DISPLAY STYLE(COLUMN)=[JUST=l FONT_FACE = calibri FONT_SIZE = 3 CELLWIDTH=17%];
DEFINE R / DISPLAY STYLE(COLUMN)=[JUST=l FONT_FACE = calibri FONT_SIZE = 3 CELLWIDTH=17%];
DEFINE RR / DISPLAY STYLE(COLUMN)=[JUST=l FONT_FACE = calibri FONT_SIZE = 3 CELLWIDTH=17%];
QUIT;
%END;
%END;
ODS _ALL_ CLOSE;
...
%Macro chtvirtualprofileemail(lineage) / MINOPERATOR;
...
%IF &Site = PGT AND &NProfile > 0 AND &PBGHour IN 5 17
	%THEN %DO;
...
%IF &Site = KMT AND &NProfile > 0 AND &KMTHour IN 6 18
	%THEN %DO;
...
%IF &Site = DMT AND &NProfile > 0 AND &DMTHour IN 5 17
	%THEN %DO;
